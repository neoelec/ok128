...
void LCD_3digit(unsigned char number)           /* display 3-digit decimal number */
{ unsigned char i;
...
{ unsigned char div_ratio;
  ...
  while(1)
    { switch(Key_input())                       // key input
        { case 0xE0 : if(div_ratio != 100)      // if SW1, decrement by 1
                        div_ratio--;
                      Set_timer1(div_ratio);
                      break;
          case 0xD0 : if(div_ratio != 240)      // if SW2, increment by 1
                        div_ratio++;
                      Set_timer1(div_ratio);
                      break;
          case 0xB0 : div_ratio -= 10;          // if SW3, decrement by 10
                      if(div_ratio < 100)
                        div_ratio = 100;
                      Set_timer1(div_ratio);
                      break;
          case 0x70 : div_ratio += 10;          // if SW4, increment by 10
                      if(div_ratio > 240)
                        div_ratio = 240;
                      Set_timer1(div_ratio);
                      break;
