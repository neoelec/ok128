/* ========================================================================== */
/*	    Exp14_2.c : Digital Clock on LCD Module and 7-Segment LED	      */
/* ========================================================================== */
...
int main(void)
{
  ...
  LCD_string(0x80," 2005/03/01 TUE ");          // display title
  ...
  if(RTC_FLAG != 'G')				// RTC data are available ?
    { RTC_FLAG = 'G';                           // initialize RTC data flag
      RTC_YEAR = 0x05;                          // initialize year(2005)
      RTC_MONTH = 0x03;                         // initialize month(03)
      RTC_DATE = 0x01;                          // initialize date(01)
      RTC_WEEKDAY = 0x03;                       // initialize weekday(TUE)
  ...
  while(1)
    { DSP_RTC();                                // display clock on LCD module
      ...
      switch(Key_input())			// key input
        { case 0x70 : Cursor_left();		// if SW4, go cursor left
                      break;
          case 0xB0 : Cursor_right();		// if SW3, go cursor right
                      break;
          case 0xD0 : Increment();		// if SW2, increment time
                      break;
          case 0xE0 : Decrement();		// if SW1, decrement time
                      break;
          default   : break;
        }
    }
